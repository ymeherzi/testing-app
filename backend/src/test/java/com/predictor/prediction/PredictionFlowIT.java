...
class PredictionFlowIT {
    ...
    @Autowired
    private com.predictor.catalog.MatchRepository matches;
    ...
    void setUp() throws Exception {
        syncService.syncAll();
        User user = users.findByEmailIgnoreCase("predictor@example.com").orElseGet(() ->
                users.save(new User("predictor@example.com", passwordEncoder.encode("correct-horse"),
                        "Predictor", "TN", null)));
        ...
        List<Long> matchIds = matches
                ...
                .stream().map(com.predictor.catalog.Match::getId).toList();

  ...
  return (
    <div className="space-y-6 p-4">
      <header className="pt-2">
        ...
        <p className="mt-1 text-sm text-slate-400">Public tables you're always in, plus your private leagues.</p>
      </header>

      <section className="space-y-2">
        <Card to="/table/global" icon="🌍" title="Global league" subtitle="Every player in the game" />
        <Card
          to="/table/country"
          icon={countryFlag(user?.country ?? null) || '🏳️'}
          title="Country league"
          subtitle={user?.country ? `Players from ${user.country}` : 'Set your country in Profile to join'}
        ...
        <Card
          ...
          icon="🛡️"
          title="Club league"
          subtitle={clubName ? `Fellow ${clubName} fans` : 'Set your favourite club in Profile to join'}
      ...
      <section className="space-y-2">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-semibold uppercase tracking-wide text-slate-500">My leagues</h2>
          <div className="flex gap-2 text-sm">
            <Link to="/table/join" className="rounded-lg bg-slate-800 px-3 py-1.5 font-medium">Join</Link>
        ...
        {leagues?.length === 0 && (
          <p className="rounded-2xl border border-dashed border-slate-700 px-4 py-6 text-center text-sm text-slate-400">
            No private leagues yet — create one and invite your friends, family or colleagues.
          </p>
        )}
        {leagues?.map((league) => (
          <Card
            ...
            icon="🏟️"
            title={league.name}
            subtitle={`${league.memberCount} members${league.myRank ? ` · you're #${league.myRank}` : ''}${
              league.admin ? ' · admin' : ''
            }`}
          />
        ))}

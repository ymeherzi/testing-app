/**
 * Every ISO 3166-1 alpha-2 country code. Display names come from
 * Intl.DisplayNames, so they arrive in the user's own language (a French
 * browser sees "Allemagne", not "Germany") and sort accordingly.
 */
const CODES = [
...
export function countries(locale: string = navigator.language): Country[] {
  ...
  const list = CODES.map((code) => ({ code, name: displayName(code, locale) })).sort((a, b) =>
    collator.compare(a.name, b.name),
  )
  cache = { locale, countries: list }
  return list
}
